...
for($l=0; $l<$i; $l++){
	for($m=0; $m<@row_array; $m++) {
			$new_val[$countN]= $main_2D_array[$l][$m];
			$countN ++;
	}
...
$workingA = ($countN / 2);
my @sorted_listN = sort {$a <=> $b} @new_val;
my $medianN = $sorted_listN[$workingA];
$nf = $medianA/$medianN;
...
for($l=0; $l<$i; $l++){
	for($m=0; $m<@row_array; $m++) {
		...
		$new_norm_total[$countN] = $main_2D_array[$l][$m];
		$countN ++;
	}

}
$workingA = ($countN / 2);
@sorted_listN = sort {$a <=> $b} @new_norm_total;
my $medianS = $sorted_listN[$workingA];
