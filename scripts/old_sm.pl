...
foreach $key ( keys %$tfData ) {		
	for($i=0;$i<$number_of_colonies_for_tf;$i++) {
		...
		my ($plateStart,$plateEnd);
		$plateStart=$plateEnd = 0;
		($plateStart,$plateEnd)=split(/-/,$plateType);
		my $marker = 0;
		if($plateStart == 1){
			$marker = 0;
		...
		my($y,$x) = split(/,/,$final_coord);
			if(exists($finalTFcalls->{$key})) {
					if(exists($tf_restricted_hash->{$key})) {
						print OUT "$filename\t$marker\t$plateType\tBLANK\tBLANK\t$key\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$median\tPositive\tBO\n";
					}
					else{
						 print OUT "$filename\t$marker\t$plateType\tBLANK\tBLANK\t$key\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$median\tPositive\n";
					}	
					
				}
				else{
					if(exists($tf_restricted_hash->{$key})) {
						print OUT"$filename\t$marker\t$plateType\tBLANK\tBLANK\t$key\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$median\tNegative\tBO\n";
					}
					else{
						print OUT "$filename\t$marker\t$plateType\tBLANK\tBLANK\t$key\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$median\tNegative\n";
					}
				}		
	}
}
