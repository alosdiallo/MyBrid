...
my $ignore_list = $ARGV[9];
my $location_m = $ARGV[10];
...
sub empty($$){
	...
	my @empty_spots_temp = split(/,/,$ignore_list);
	print " I am here $empty_spots_temp[0]\n";
	if ($empty_spots_temp[0] ne "Blank") {
		@empty_spots = @empty_spots_temp;
	}
	else{
		@empty_spots = ("");
		print " I am here $empty_spots_temp[0]\n";
...
sub zscore_prime ($$$$$){
	my $main_array_zscore_ref = shift;
	my $row_array_ref = shift;
	my $i = shift;
	...
	my @zprime;
	my @row_array = @$row_array_ref;
	my @main_array_zscore = @$main_array_zscore_ref;
	my ($l,$m);
	
	for($l=0; $l<$i; $l++){
		for($m=0; $m<@row_array; $m++){
			$zprime[$l][$m] = $main_array_zscore[$l][$m] / $top_row_median_value;
			$zprime[$l][$m] = ($zprime[$l][$m]) * 1000;
			#print OUT "$zprime[$l][$m]\n";
			$zprime[$l][$m] = $zprime[$l][$m] / 8.346124;
			#$zprime[$l][$m] = $zprime[$l][$m] / 0.1734923;
			#$zprime[$l][$m] = $main_array_zscore[$l][$m] * ($zmedian / (0.144333333*$top_row_median_value))
		}
	}
	return(\@zprime);
}
...
sub tfHash_Sub($$$$$$$$$$$$$$){
	...
	for($l=0; $l<$i; $l++){
		for($m=0; $m<@row_array; $m++) {
			...
			$tfData{$key}{'zscorePrime'}[$index]=$zscore_prime;
...
sub restricted_Hashes($$$){
	...
	foreach $key ( keys %$tfData ) {	
		...
		for($i=0;$i<$number_of_colonies_for_tf;$i++) {
			...
			my $zscore_prime = $tfData->{$key}->{'zscorePrime'}[$i];
		...
		for($posThreshold=0;$posThreshold<=10;$posThreshold=$posThreshold+$incrementor) {
			...
			foreach $tfName (keys %$tfCount ) {	
				...
				for($i=0;$i<$nHits;$i++) {
					...
					my $zscore_prime=$tfData->{$tfName}->{'zscorePrime'}[$i];
...
sub shortList($$$$$$$$$$$){
	...
	foreach $tfName (keys %$tfCount ) {	
		...
		for($i=0;$i<$number_of_colonies_for_tf;$i++) {
			...
			$zscore_prime = $tfData->{$tfName}->{'zscorePrime'}[$i];
			...
			if($zscore_prime >= $chosen_cut_off) {
				$nPos++;
			}
			$no_non_ignored++;
		}
	...
	foreach $tfName (keys %finalTFcalls ) {
		...
		for($i=0;$i<$number_of_colonies_for_tf;$i++) {
			...
			if((!exists($ignore->{$coord})) and ($zscore_prime >= 2.14)) {
				$total_raw_value = $raw_value + $total_raw_value;
				$total_raw_zscore = $total_raw_zscore + $zscore_prime;
		...
		for($i=0;$i<$number_of_colonies_for_tf;$i++) {
			...
			if((!exists($ignore->{$coord})) and ($zscore_prime >= 2.14)) {
			$deviation_score = $zscore_prime - $avg_raw_zscore;
	...
	# }
		if (exists($bait_more_info->{$filename})) {
			if (exists($tf_more_info->{$tfName})) { 
				if (!exists($empty_bait->{$filename})) {
					if (exists($tf_Limited_restricted_hash->{$tfName})) {
						#print OUT "$baitName\t$genename\t$nPos\t$avg_norm\t$avg_raw_zscore\t$avg_raw_zscore_normal\t$avg_raw_value\t$avg_size\t$top_row_median_value\tBO\n";
					}
					#else {print OUT "$baitName\t$genename\t$nPos\t$avg_norm\t$avg_raw_zscore\t$avg_raw_zscore_normal\t$avg_raw_value\t$avg_size\t$top_row_median_value\n";}
				}
			}
		}
	}
...
foreach $key ( keys %$tfData ) {	
		...
		for($i=0;$i<$number_of_colonies_for_tf;$i++) {
			...
			# if(exists($tf_more_info->{$key})) {
					# $tffile = $tf_more_info->{$key}; 
					# my ($arraytfname,$orf,$genename) = split(/\|/,$tffile);
					# if(exists($finalTFcalls->{$key})) {
						# if(exists($tf_restricted_hash->{$key})) {

							# print OUT "$filename\t$plateType\t$key\t$orf\t$genename\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$zscore_prime\t$median\tPositive\tBO\n";
						# }
						# else{
							 # print OUT "$filename\t$plateType\t$key\t$orf\t$genename\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$zscore_prime\t$median\tPositive\n";
						# }	
						
					# }
					# else{
						# if(exists($tf_restricted_hash->{$key})) {
							# print OUT "$filename\t$plateType\t$key\t$orf\t$genename\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$zscore_prime\t$median\tNegative\tBO\n";
						# }
						# else{
							# print OUT "$filename\t$plateType\t$key\t$orf\t$genename\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$zscore_prime\t$median\tNegative\n";
						# }
					# }
				...
				# else{
					if(exists($finalTFcalls->{$key})) {
							if(exists($tf_restricted_hash->{$key})) {
								print OUT "$filename\t$plateType\t$key\tBLANK\tBLANK\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$zscore_prime\t$median\tPositive\tBO\n";
							}
							else{
								 print OUT "$filename\t$plateType\t$key\tBLANK\tBLANK\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$zscore_prime\t$median\tPositive\n";
							}	
							
						}
						else{
							if(exists($tf_restricted_hash->{$key})) {
								print OUT"$filename\t$plateType\t$key\tBLANK\tBLANK\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$zscore_prime\t$median\tNegative\tBO\n";
							}
							else{
								print OUT "$filename\t$plateType\t$key\tBLANK\tBLANK\t$y\t$x\t$originalVal\t$raw_value_new\t$normal_value\t$final_zscore\t$zscore_prime\t$median\tNegative\n";
							}
						}		
				#}
...
($empty_spots_tf,$empty_bait)=&empty($location_m,$ignore_list);
#($empty_spots_tf,$empty_bait)=&empty(); OLD
...
($zmedian) = &zmedian($plateType);

($zprime) = &zscore_prime(\@zscore_array_final,\@row_array,$i,$top_row_median_value,$zmedian);
@zprime = @$zprime;
...
($tfCount,$tfLookUp,$tfData)=&tfHash_Sub(\@main_2D_array,\@main_2D_tfarray,\@zscore_array_final,\@row_array,\@main_2D_size_array,$i,\@reduced_val_array,$bait_more_info,$tf_more_info,$filename,$empty_bait,\@original_values,\@zprime,\@main_2D_arrayN);
