...
class Data_Model extends CI_Model{
	...
	public function getPlateViewData($user, $project){
		$str = "SELECT DISTINCT Promoter.*, Interactions.plate_number FROM Promoter, Interactions WHERE Interactions.plate_name = Promoter.bait_id AND Promoter.user_id = '$user' AND Interactions.user_id = '$user' AND Promoter.project_id = '$project' AND Interactions.project_id = '$project' GROUP BY Promoter.bait_id ORDER BY Interactions.plate_name";
		//echo $str;
...
//		}
		...
		if($loadData['promoterSearch']){
			$str .= " ORDER BY $this->interactionTable.plate_name, $this->interactionTable.plate_number";
		}
		//
		// Complex Query stuff
		//
		//
		// Grab the tf list and attach it to the promoter information for later usages
		//
		$list = $this->getTranscriptionFactorList($loadData['userId'], $loadData['projectId']);	
