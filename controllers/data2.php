...
class Data extends CI_Controller{
	...
	*/
				...
				if($positiveSearch == "true"){
					...
					foreach($promoterQuery->result() as $row){
						...
						if(($bait_old != $row->bait_id) || ($plateNumber_old != $row->plate_number)){
							...
							if($promoterSearch && !isset($promoterResult[$promoterResultsLoop]->transcriptionData)){
	...
	public function setImages($data){
		...
		//print_r($data);
		foreach($data as &$id){
			if(isset($id->user_id) || isset($id->project_id) || isset($id->bait_id) || isset($id->plate_number)){
		...
		foreach($users as $user){
			...
			foreach($promoters as $promoter){
				if($promoter == ""){
					...
					if($andSearch == "TRUE"){
						$promoterDataResults = array_merge($promoterDataResults, $this->promoterSearch("ALL", $user, $project, $positiveSearch, $transcriptionFactorData, true, $andSearch));
					}
					// Do Nothing?
				} else {
					$promoterDataResults = array_merge($promoterDataResults, $this->promoterSearch($promoter, $user, $project, $positiveSearch, $transcriptionFactorData, true, $andSearch));
